...
def get_user_key(): return f"user_{session['user_id']}" if 'user_id' in session else get_remote_address()
...
def init_db():
    ...
    # NEW: Folder System Tables for Batch Queue
    c.execute("CREATE TABLE IF NOT EXISTS PendingFolders (id SERIAL PRIMARY KEY, name TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);")
    c.execute("CREATE TABLE IF NOT EXISTS PendingImages (id SERIAL PRIMARY KEY, folder_id INTEGER REFERENCES PendingFolders(id) ON DELETE CASCADE, image_path TEXT, original_filename TEXT);")
    ...
    if not c.fetchone(): c.execute("INSERT INTO Users (username, password, role) VALUES ('shafin', %s, 'admin')", (generate_password_hash('29743115'),))
...
def gallery_settings():
    if request.method == 'POST':
        ...
        settings = get_settings(); settings["gallery_visible"] = request.json.get("visible", True)
...
def handle_mode():
    ...
    if request.method == 'POST':
        ...
        conn.commit(); mode = request.json.get('mode')
    else:
...
@admin_required
def get_users():
    conn = get_db_connection(); c = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
...
@admin_required
def add_user():
    data = request.json; hashed_pw = generate_password_hash(data['password'])
    conn = get_db_connection(); c = conn.cursor()
    try:
        ...
        conn.rollback(); return jsonify({"error": "Username already exists."}), 400
    finally: conn.close()
...
def delete_user(user_id):
    ...
    if user_id == master_id: conn.close(); return jsonify({"error": "Action Denied: You cannot delete the Master Admin account."}), 403
...
def update_master_admin():
    data = request.json
    current_user = data.get('current_username'); current_pass = data.get('current_password')
    new_user = data.get('new_username'); new_pass = data.get('new_password')
    
    conn = get_db_connection(); c = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
    c.execute("SELECT id, username, password FROM Users ORDER BY id ASC LIMIT 1")
    master = c.fetchone()
    
    if not master: conn.close(); return jsonify({"error": "Master admin not found"}), 404
    if current_user != master['username'] or not check_password_hash(master['password'], current_pass):
        conn.close(); return jsonify({"error": "Verification failed. Incorrect current username or password."}), 403
    ...
    try:
        ...
        conn.commit(); status, err = "success", None
    except psycopg2.IntegrityError:
        conn.rollback(); status, err = "error", "That new username is already taken by another user."
    conn.close()
...
def analyze_image_core(file_path, mode, manual_rating_val=None):
    current_rating = 0.0; comment = "Manually rated."; emotion = "Neutral"; age = 0
    if mode == 'ai':
        if FACEPP_API_KEY:
            try:
                ...
                else: comment = "No face detected by Cloud AI."
            except Exception as e:
                print(f"Face++ Error: {e}"); comment = "Cloud AI timeout. Fallback used."; current_rating = round(random.uniform(5.5, 8.5), 1)
        else:
            current_rating = round(random.uniform(6.0, 9.0), 1)
            emotion = random.choice(["Happy", "Neutral", "Surprise", "Calm"]); age = random.randint(18, 45)
...
def rate():
    ...
    c.execute("SELECT value FROM Settings WHERE key = 'rating_mode'"); mode = c.fetchone()[0]
    results = []
...
@app.route('/api/pending/folders', methods=['POST'])
@admin_required
def create_pending_folder():
    folder_name = request.form.get('folder_name')
    files = request.files.getlist('images')
    
    if not folder_name or not files: return jsonify({"error": "Missing folder name or images"}), 400
    if len(files) > 100: return jsonify({"error": "Maximum 100 images allowed per folder"}), 400

    conn = get_db_connection(); c = conn.cursor()
    c.execute("INSERT INTO PendingFolders (name) VALUES (%s) RETURNING id", (folder_name,))
    folder_id = c.fetchone()[0]
    
    count = 0
    for file in files:
        if file.filename == '' or not file.filename.lower().endswith(('.png', '.jpg', '.jpeg', '.webp')): continue
        filename = secure_filename(file.filename)
        file_path = os.path.join(UPLOAD_FOLDER, f"q_{folder_id}_{int(time.time())}_{filename}")
        file.save(file_path)
        c.execute("INSERT INTO PendingImages (folder_id, image_path, original_filename) VALUES (%s, %s, %s)", 
                  (folder_id, file_path.replace('public', ''), file.filename))
        count += 1
        
    conn.commit(); conn.close()
    return jsonify({"status": "success", "uploaded": count})

@app.route('/api/pending/folders', methods=['GET'])
@login_required
def get_pending_folders():
    conn = get_db_connection(); c = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    c.execute("""
        SELECT f.id, f.name, COUNT(i.id) as image_count 
        FROM PendingFolders f JOIN PendingImages i ON f.id = i.folder_id 
        GROUP BY f.id, f.name ORDER BY f.created_at ASC
    """)
    data = c.fetchall(); conn.close()
    return jsonify(data)

@app.route('/api/pending/folders/<int:folder_id>/images', methods=['GET'])
@login_required
def get_folder_images(folder_id):
    conn = get_db_connection(); c = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    c.execute("SELECT * FROM PendingImages WHERE folder_id = %s ORDER BY id ASC", (folder_id,))
    data = c.fetchall(); conn.close()
    return jsonify(data)

@app.route('/api/pending/folders/<int:folder_id>', methods=['DELETE'])
@admin_required
def delete_folder(folder_id):
    conn = get_db_connection(); c = conn.cursor()
    c.execute("SELECT image_path FROM PendingImages WHERE folder_id = %s", (folder_id,))
    for row in c.fetchall():
        try: os.remove(os.path.join('public', row[0].lstrip('/')))
        except: pass
    c.execute("DELETE FROM PendingFolders WHERE id = %s", (folder_id,))
    conn.commit(); conn.close()
    return jsonify({"status": "success"})

@app.route('/api/pending/rate_single', methods=['POST'])
@login_required
def rate_single_pending():
    data = request.json
    image_id = data.get('image_id'); folder_name = data.get('folder_name'); manual_rating = data.get('rating')

    if not image_id or not folder_name: return jsonify({"error": "Missing data"}), 400
    conn = get_db_connection(); c = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)

    c.execute("SELECT image_path, folder_id FROM PendingImages WHERE id = %s", (image_id,))
    img = c.fetchone()
    if not img: conn.close(); return jsonify({"error": "Image not found"}), 404

    c.execute("SELECT id FROM Persons WHERE name = %s", (folder_name,))
    row = c.fetchone()
    p_id = row[0] if row else None
    if not p_id:
        c.execute("INSERT INTO Persons (name) VALUES (%s) RETURNING id", (folder_name,))
        p_id = c.fetchone()[0]

    c.execute("SELECT value FROM Settings WHERE key = 'rating_mode'"); mode = c.fetchone()[0]

    full_path = os.path.join('public', img['image_path'].lstrip('/'))
    current_rating, comment, emotion, age = analyze_image_core(full_path, mode, manual_rating)

    c.execute("INSERT INTO Ratings (person_id, rating, image_path, comment, emotion, age_estimate) VALUES (%s, %s, %s, %s, %s, %s)", 
              (p_id, current_rating, img['image_path'], comment, emotion, age))
    
    c.execute("DELETE FROM PendingImages WHERE id = %s", (image_id,))
    
    c.execute("SELECT COUNT(*) FROM PendingImages WHERE folder_id = %s", (img['folder_id'],))
    if c.fetchone()[0] == 0: c.execute("DELETE FROM PendingFolders WHERE id = %s", (img['folder_id'],))
...
def get_stats():
    ...
    c.execute("SELECT COUNT(*) as pending_count FROM PendingImages")
    try: pending_count = c.fetchone()['pending_count']
    except: pending_count = 0
    conn.close()
    return jsonify({ "total_subjects": total_subjects, "total_ratings": ratings_data['total_ratings'], "avg_rating": float(ratings_data['avg_rating']), "total_users": total_users, "pending_count": pending_count })
...
def require_api_key(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not api_key or api_key != PUBLIC_API_KEY: return jsonify({"error": "Invalid API key"}), 401
        return f(*args, **kwargs)
    return decorated
